		...
		<Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
		    ...
		    <FormItem label="爆品等级" prop="explosiveLevel">
		        <Select v-model="formValidate.explosiveLevel" placeholder="请选择爆品等级">
		          <Option :value="item.dictName" v-for="item in optionType.explosiveList">{{item.dictName}}</Option>
		        </Select>
		    </FormItem>
		    <FormItem label="开发进度" prop="developProgress">
		        <Select v-model="formValidate.developProgress" placeholder="请选择开发进度">
		          <Option :value="item.dictName" v-for="item in optionType.developProgressList">{{item.dictName}}</Option>
		        </Select>
		    </FormItem>
		    <FormItem label="新品类型" prop="newProductType">
		        <Select v-model="formValidate.newProductType" placeholder="请选择新品类型">
		           <Option :value="item.dictName" v-for="item in optionType.newProductList">{{item.dictName}}</Option>
		        </Select>
		    </FormItem>
			...
			getDetail() {
              ...
              that.ajax('/goods/getGoodsDetail',{projectId: that.proId}).then(res=>{
              	 if(res.code===200) {
			        ...
			        // if(res.data.selectionFlag === '1') {
			        // 	that.editValidate.selectionFlag = true
			        // } else {
			        // 	that.editValidate.selectionFlag = false
			        // }
              	 }
              })
			},
