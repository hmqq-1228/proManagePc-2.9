      ...
      <div v-if="goodList.length > 0" class="goodItem" v-for="(good, index) in goodList" v-bind:key="index">
        <div class="goodItemCon">
          ...
          <div class="goodInfo" @click="toGoodsManage(good.projectId)">
            ...
            <div>品牌: <span v-if="good.categoryName">{{good.categoryName}}</span><span v-if="!good.categoryName" style="color: #999;font-size: 12px;">未分类</span></div>
            <div>负责人: <span>{{good.developProgress}}</span></div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    bbbb: function () {
      console.log('level0000', this.level_0)
    },
    ...
    secondType: function (e, name, type, code) {
      var that = this
      that.routerList2 = JSON.parse(localStorage.getItem('setRouter'))
      // that.routerList2 = that.setRouterArr
      console.log('888888888', that.routerList2)
      var obj = {
        categoryType: type,
        categoryName: name,
        code: code,
      ...
      if (name === '全部') {
        ...
        if (that.routerList2 && that.routerList2.length > 0) {
          var flag = false
          var index
          for (var n = 0; n < that.routerList2.length; n++) {
            if (that.routerList2[n].codeLen === code.length) {
              index = n
              flag = true
            }
          }
          if (flag) {
            that.setRouterArr.splice(index, that.routerList2.length, obj)
          } else {
            that.setRouterArr.push(obj)
          }
        } else {
          that.setRouterArr.push(obj)
        }
        localStorage.setItem('setRouter', JSON.stringify(that.setRouterArr))
      }
      that.currentPage = 1
      that.getGoodList.pageNum = 1
      var codeLen = ''
